...
options:
    command:
        description: To configure a specific feature of NTP -
          ...
          choice enable_includeCredentials allows you to enable
                include credentials on the switch, either Encrypt
                Credentials or Include Credentials must be enabled
                to use key-id authentication with this module
          choice config_ntp allows you to enable/disable and
                configure switch's global NTP settings
          choice config_ntp_ipv4addr allows you to configure an NTP server
                with IPv4 address or hostname
          choice config_ntp_keyId allows you to configure an
...
def config_ntp(module):
    ...
    if params['trap_value'] is not None:
        ...
        for item in params['trap_value']:
            ...
            if 'enable' in item and item['enable'] is False:
...
def config_ntp_ipv4addr(module):
    ...
    if params['keyId'] and params['keyId'] != 0:
        # This returns an empty list if encrypt credentials is not enabled
        # Verify the key is trusted
        check_presence = get_config(module,
                                    "/config/ntp/authentication/key-id/int/"
                                    + str(params['keyId']))
        if not check_presence:
            return {
                'msg': 'Authentication key-id {} needs to be configured '
        ...
        for key in data:
            if key == "ASCII-STR":
                ...
                newdata[key][ip4_addr_val] = \
                    newdata[key][ip4_addr_val].strip('\"')
    ...
    # When address is in host name format, "check" in run_commands
    # will not work. so adding the additional check to verify idempotency
    if ip4_addr == "ASCII-STR" and \
            diffseen is False and method != 'DELETE':
