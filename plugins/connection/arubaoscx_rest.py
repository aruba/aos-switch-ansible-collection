...
from ansible.module_utils.six.moves import cPickle # NOQA
from ansible.playbook.play_context import PlayContext # NOQA
from ansible.plugins.loader import connection_loader # NOQA
from ansible.plugins.connection import ConnectionBase # NOQA
from ansible.utils.path import unfrackpath # NOQA
...
class Connection(ConnectionBase):
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        super(Connection, self).__init__(play_context,
                                         new_stdin, *args, **kwargs)

        self._is_http = True
        self._history = list()

        self._local = connection_loader.get('local',
                                            self._play_context,
                                            '/dev/null')
    ...
    def _construct_urls(self):
        ...
        self._login_url = '%s://%s%s' % (self._protocol,
                                         self._remote_host,
                                         login_string)
        self._logout_url = '%s://%s%s' % (self._protocol,
                                          self._remote_host,
                                          logout_string)
        self._run_config_url = '%s://%s%s' % (self._protocol,
                                              self._remote_host,
                                              run_config_string)
    ...
    def _connect(self):
        ...
        payload = {'action': 'login', 'username': self._username,
                   'password': self._password}
        self._http_session_handle = requests.session()
        self._http_session_handle.trust_env = False
        response = self._http_session_handle.post(self._login_url,
    ...
    def reset(self):
        ...
        if self._socket_path:
            display.vvvv('resetting persistent connection for socket_path %s'
                         % self._socket_path, host=self.host)
            self.close()
        display.vvvv('reset call on connection instance', host=self.host)

    def close(self):
        if self._connected:
            response = self._http_session_handle.post(self._logout_url,
                                                      verify=False)
    ...
    def get_running_config(self):
        if self._connected and self._http_session_handle:
            response = self._http_session_handle.get(self._run_config_url,
                                                     verify=False)
            return response.text

    def put_running_config(self, updated_config):
        if self._connected and self._http_session_handle:
            response = self._http_session_handle.put(self._run_config_url,
                                                     data=updated_config,
                                                     verify=False)
            return response.text
